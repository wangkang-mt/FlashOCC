...
def test_bev_pool_v2():
    ...
    depth = torch.from_numpy(depth).float().musa()
    depth = depth.view(1, 1, 2, 2, 2).requires_grad_()
    feat = torch.ones(
        size=[1, 1, 2, 2, 2], dtype=torch.float,
        device='musa').requires_grad_()
    ranks_depth = torch.from_numpy(np.array([0, 4, 1, 6])).int().musa()
    ranks_feat = torch.from_numpy(np.array([0, 0, 1, 2])).int().musa()
    ranks_bev = torch.from_numpy(np.array([0, 0, 1, 1])).int().musa()
    ...
    grad_depth = grad_depth.musa().view(1, 1, 2, 2, 2)
    assert depth.grad.allclose(grad_depth)
    grad_feat = np.array([1.0, 1.0, 0.4, 0.4, 0.8, 0.8, 0., 0.])
    grad_feat = torch.from_numpy(grad_feat).float().musa().view(1, 1, 2, 2, 2)
